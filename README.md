...
- **Rating.** Each objective has a **par** — the optimal cost to reach the target and return (`80` for the default `Q2` objective: two `shift`s out and two back). On winning you earn a **1–3 star efficiency rating**: three stars for finishing at or under par, two within twice par, and one for any slower win. Par is shown from the start so you know the score to beat.

`where` shows your **budget remaining**, **objective status**, and **par** (plus your rating once won) at every step, and the browser HUD shows a budget chip (red when depleted) and an objective badge (pending → reached → won, with par and your star rating on the win).
...
- an optional [game mode](#game-mode) (on by default): a finite budget that refuses unaffordable moves, a reach-the-target-and-return-home win condition, and a par-based 1–3 star efficiency rating on winning, surfaced in `where` and the browser HUD
