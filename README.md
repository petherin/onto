...
- **Objective.** You are given a **quest chain**: an ordered list of target coordinates you must reach **in order** before returning home. The default chain has two waypoints on different reality axes — first the second quantum branch of home (`Q2`), then one simulation layer deep (`sim:1`). Reaching a mid-chain waypoint announces the next one ("Objective 1 of 2 reached — next: reach …"); reaching the last announces "Objective reached — now return home to win"; getting home once the whole chain is done announces "You win!". A single-objective quest is just a chain of length one.
- **Rating.** Each quest has a **par** — the optimal cost to visit every waypoint in order and return (`140` for the default chain: two `shift`s out and two back, then `simulate` in and out). On winning you earn a **1–3 star efficiency rating**: three stars for finishing at or under par, two within twice par, and one for any slower win. Par is shown from the start so you know the score to beat.

`where` shows your **budget remaining**, the **objective checklist** (each waypoint ticked as it is reached), your progress and **par** (plus your rating once won) at every step, and the browser HUD shows a budget chip (red when depleted) and an objective badge (progress `N/M` → all reached → won, with par and your star rating on the win).
...
- an optional [game mode](#game-mode) (on by default): a finite budget that refuses unaffordable moves, an ordered multi-objective quest chain (reach every waypoint in order, then return home to win), and a par-based 1–3 star efficiency rating on winning, surfaced in `where` and the browser HUD
